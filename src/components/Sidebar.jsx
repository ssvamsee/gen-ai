...
export default function Sidebar({ activeCategory, setActiveCategory, activeTopic, setActiveTopic, topics, searchQuery, setSearchQuery, isOpen, setIsOpen }) {
    ...
    return (
        <>
            ...
            <div className={`fixed lg:relative h-full transition-all duration-300 ${isOpen ? 'w-80 translate-x-0 opacity-100' : 'w-0 lg:w-20 lg:translate-x-0 -translate-x-full opacity-0 lg:opacity-100'} glass-panel border-r border-slate-800 shrink-0 z-50 overflow-hidden lg:overflow-visible`}>
                    ...
                    {isOpen && (
                        <div className="px-6 mb-4 relative">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-900/50 border border-slate-800 rounded-lg py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30 transition-all placeholder:text-slate-500 text-slate-200"
                            />
                        </div>
                    )}
                            ...
                            Object.keys(groupedTopics).map((subCat) => (
                                <div key={subCat} className="mb-4">
                                    <button
                                        ...
                                        className="w-full flex items-start justify-between p-2 mb-1 hover:bg-slate-800/30 rounded-lg transition-all group"
                                    >
                                        <span className="text-[11px] font-black text-slate-300 tracking-[0.15em] uppercase group-hover:text-white transition-colors text-left">
                                            {subCat}
                                        </span>
                                                ...
                                                <div className="space-y-1 mt-1">
                                                    {groupedTopics[subCat].map((topic) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div>
                                                                <p className={`text-sm font-medium ${activeTopic?.id === topic.id ? 'text-white' : 'text-slate-200 group-hover:text-white'}`}>
                                                                    {topic.title}
                                                                </p>
                                                                {topic.short_ref && (
                                                                    <p className="text-[10px] text-slate-400 mt-0.5 line-clamp-1 group-hover:text-slate-300 font-normal">
                                                                        {topic.short_ref}
                                                                    </p>
                                                                )}
                                                            </div>
